...
def plan_app_spec(idea: str, packages_extra: List[str] | None = None) -> AppSpec:
    ...
    messages = [
        {
            "role": "system",
            "content": "You turn app ideas into minimal JSON specifications.",
        },
        {"role": "user", "content": PROMPT.format(idea=idea)},
    ]
    result = call_openai(model=sel["model"], messages=messages, **sel.get("params", {}))
    ...
    try:
        ...
        result = call_openai(
            model=sel["model"], messages=retry_messages, **sel.get("params", {})
        )
        content = result["text"] or ""
        try:
            data = json.loads(content)
    ...
    spec = AppSpec(
        ...
        python_packages=list(
            dict.fromkeys(data.get("python_packages", []) + (packages_extra or []))
        ),
    )
    return spec
