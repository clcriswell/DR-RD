...
def test_orchestrator_iterative_loop_executes_all_roles(tmp_path):
    ...
    with patch.object(orch, "PlannerAgent", DummyPlanner), patch.object(
        orch, "build_agents", fake_build_agents
    ), patch.object(orch, "synthesize", return_value="final"), patch.object(
        orch,
        "load_mode_models",
        return_value={"Planner": "x", "synth": "x", "default": "x"},
    ):
        final, results, trace = orch.run_pipeline(
            "idea", mode="test", session_id="s", runs_dir=tmp_path
        )
